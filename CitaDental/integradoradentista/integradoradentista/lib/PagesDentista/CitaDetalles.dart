...
class _CitaDetallesState extends State<CitaDetalles> {
  ...
  Future<String> obtenerNombrePaciente(String idPaciente) async {
    try {
      final pacienteSnapshot = await databaseRef.child('pacientes/$idPaciente').once();
      if (pacienteSnapshot.snapshot.exists) {
        final dentistaData = pacienteSnapshot.snapshot.value as Map<dynamic, dynamic>;
        return dentistaData['nombre'] ?? 'Paciente desconocido';
      } else {
  ...
  Future<String> obtenerNombreDentista(String idDentista) async {
    try {
      final dentistaSnapshot =
      await databaseRef.child('dentistas/$idDentista').once();

      if (dentistaSnapshot.snapshot.exists) {
        final pacienteData =
        dentistaSnapshot.snapshot.value as Map<dynamic, dynamic>;
                  ...
                  children: [
                    Row(
                      children: [
                        ...
                        Text(
                          'Paciente: ${nombrePaciente ?? 'Sin asignar'}',
                    ...
                    Row(
                      children: [
                        ...
                        const SizedBox(width: 8),
                        Text(
                          'Dentista: ${nombreDentista ?? 'Desconocido'}',
