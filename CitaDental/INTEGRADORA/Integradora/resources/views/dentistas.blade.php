@extends('adminlte::page')
@section('title', 'Dentistas')
    
@section('content')
<header>
</header>

<br>
<center>
    <div class="text-center mb-3">
        <img src="{{ asset('ClinicControlIcono.png') }}" alt="Logo" class="img-fluid" style="max-width: 90px; height: auto;">
    </div>
    <div>
        <h3 class="text-center">Dentistas</h3>
    </div>
</center>

<div class="box-body">
    <table id="table-data" style="width: 90%;" class="animate__animated animate__bounce table table-bordered custom-table">
        <thead  style="background-color:#1338BE; color:#fff">
            <tr>
                <th>Nombre</th>
                <th>Cédula</th>
                <th>Teléfono</th>
                <th>Correo</th>
                <th style="width:12%; height:22%;"  colspan="2">Opciones</th>
            </tr>
        </thead>
        <tbody style=" background-color:rgb(255,255,255)">
            @foreach($dentistaList as $dentista)
            <tr>
                <td>{{ $dentista['nombre'] }}</td>
                <td>{{ $dentista['cedula'] }}</td>
                <td>{{ $dentista['telefono'] }}</td>
                <td>{{ $dentista['correo'] }}</td>
                
                <td>
                    <a href="{{ route('dentista.nuevo', ['id' => $dentista['id']]) }}" class="btn btn-success btn-sm rounded-8 lote">
                    <span><img src="data:image/png;base64,"/></span>
                    </a>
                </td>
                <td>
                    <form action="{{ route('dentista.borrar', ['id' => $dentista['id']]) }}" class="formulario-eliminar" method="POST">
                        @csrf
                        @method('DELETE')
                        <button type="submit" style="border-radius: 10px;" class="btn btn-danger btn-sm rounded-8 animate__delay-1s">
                        <span><img src="data:image/png;base64,"/></span>
                    </button>
                    </form>
                </td>   
            </tr>
            @endforeach
        </tbody>
    </table>
</div>

@endsection

@section('js')
<script>
    $('#table-data').DataTable({
        "scrollX": true
    });
</script>
<script>
    $('.formulario-eliminar').submit(function(e) {
        e.preventDefault(); // Previene el envío normal del formulario
        Swal.fire({
            title: "Estas seguro?",
            text: "No podras recuperar el dato eliminado",
            icon: "warning",
            showCancelButton: true,
            confirmButtonColor: "#3085d6",
            cancelButtonColor: "#d33",
            confirmButtonText: "Confirmar"
        }).then((result) => {
            if (result.isConfirmed) {
                this.submit(); // Envía el formulario después de la confirmación
            
                Swal.fire({
                    title: "Eliminado",
                    text: "Se elimino de forma correcta",
                    icon: "success"
                });
               }
        });
    });
</script>
<script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>
@stop

@section('css')
<style>
body {
    background-color: #c1e8ff;
}

.btn:hover {
    transform: scale(1.1);
}

table {
    background: white;
    width: 50%;
    margin: 0 auto;
    margin-top: 2%;
    border-collapse: collapse;
    text-align: center;
}

.custom-table {
    border-collapse: collapse; 
    border: 1px solid #ddd; 
    box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); 
    border-radius: .4em;
    overflow: hidden;
}

.custom-table th,
.custom-table td {
    border: none; 
    padding: 8px; 
    padding: 10px;
}

.custom-table tr {
    border-bottom: 1px solid #ddd; 
}

tr:hover {
    background-color: #d2ede3; 
}
</style>
@stop
